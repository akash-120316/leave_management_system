...
include ("sql.php");
     ...
     $sql = "SELECT * FROM leavedetail WHERE email='akash16.04.2005@gmail.com'";
     ...
     if ($result = mysqli_query($link, $sql)) {
          ...
          if (2 == 2) {
            $add_to_db = mysqli_query($link, "UPDATE leaveinfo SET status='Accepted' WHERE email='" . $eid . "' AND reason='" . $descr . "' AND startdate='" . $st . "'");
          
            if ($add_to_db) {
              if ($_SERVER["REQUEST_METHOD"] == "POST") {
                $uname = $_REQUEST['message'];
                if (2 == 2) {
                  $add_to_db = mysqli_query($link, "UPDATE leaveinfo SET remarks='$uname' WHERE email='" . $eid . "' AND reason='" . $descr . "' AND startdate='" . $st . "'");
          
                  if ($add_to_db) {
                    $to_email = "akash16.04.2005@gmail.com";
                    $subject = "YOUR LEAVE REQUEST HAS BEEN ACCEPTED";
                    $body = "GREETINGS $EID
           THE $lty LEAVE REQUEST THAT YOU'VE APPLIED FOR $st FOR A PERIOD OF $daysDifference 
           FOR THE REASON: $descr  HAS BEEN [-ACCEPTED-] BY THE AUTHORITY 
          ...
          if($cas==0){
            $sql = "UPDATE leavedetail SET lop = lop + $daysDifference WHERE email='" . $eid . "'";
            if (mysqli_query($link, $sql)) {
            }
          }
          else{
                      $sql = "UPDATE leavedetail SET casualleave = casualleave - $daysDifference WHERE email='" . $eid . "'";
                      if (mysqli_query($link, $sql)) {
                      }}
                    }
                    ...
                    if ($lty == "sick") {
                      ...
                      $sql = "UPDATE leavedetail SET sickleave = sickleave + $daysDifference WHERE email='" . $eid . "'";
                      if (mysqli_query($link, $sql)) {
                      }
                    }
                    if ($lty == "vacation") {
                      $sql = "UPDATE leavedetail SET vacation = vacation + $daysDifference WHERE email='" . $eid . "'";
                      if (mysqli_query($link, $sql)) {
                      }
                    }
